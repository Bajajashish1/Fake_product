...
class HistoryManager:
    ...
    def load_history(self, limit=None, filter_by=None):
        ...
        try:
            # If no limit specified, get all records
            actual_limit = 1000 if limit is None else limit
            return self.db_manager.get_analysis_history(limit=actual_limit, filter_by=filter_by)
        except Exception as e:
    ...
    def save_flagged(self, product_id):
        """Flag or unflag a product"""
        try:
            # Get current state to toggle
            current = self.get_analysis_by_id(product_id)
            if current:
                new_state = not current.get('is_flagged', False)
                return self.update_analysis(product_id, {'is_flagged': new_state})
            return False
        except Exception as e:
            st.error(f"Error flagging product: {str(e)}")
            return False
    
    def get_statistics(self, days=None):
        """Get analysis statistics"""
        try:
            # Get all-time statistics if days is None
...
class FakeProductDetector:
    ...
    def analyze_with_ai_api(self, image):
        ...
        max_retries = 5
        retry_delay = 5  # seconds - increased for rate limit handling
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        wait_time = (retry_delay * 3) * (2 ** attempt)  # Longer delays for rate limit
                        remaining_attempts = max_retries - attempt - 1
                        print(f"DEBUG: Rate limited, retrying in {wait_time} seconds (attempt {attempt + 1}/{max_retries})...")
                        st.warning(f"⏱️ Rate limit hit. Retrying in {wait_time}s... (Attempt {attempt + 1}/{max_retries})")
                        time.sleep(wait_time)
                        continue
                    else:
                        return None, "⏱️ Rate limit exceeded. Please wait 2-3 minutes and try again. Google Gemini API has strict rate limits for free tier usage."
...
def main():
    ...
    with col1:
        if st.button("� Smart Scan", use_container_width=True, type="primary" if st.session_state.current_tab == 'scan' else "secondary", key="smart_scan_btn"):
            st.session_state.current_tab = 'scan'
    
    with col2:
        if st.button("📋 History", use_container_width=True, type="primary" if st.session_state.current_tab == 'history' else "secondary", key="history_btn"):
            st.session_state.current_tab = 'history'
            if 'history_manager' not in st.session_state:
                st.session_state.history_manager = HistoryManager()
            # Load all history with no limit
            st.session_state.analysis_history = st.session_state.history_manager.load_history(limit=None)
    
    with col3:
        if st.button("📊 Analytics", use_container_width=True, type="primary" if st.session_state.current_tab == 'analytics' else "secondary", key="analytics_btn"):
            st.session_state.current_tab = 'analytics'
            if 'history_manager' not in st.session_state:
                st.session_state.history_manager = HistoryManager()
            # Get all-time statistics
            stats = st.session_state.history_manager.get_statistics(days=None)
            
    with col4:
        if st.button("⚙️ Settings", use_container_width=True, type="primary" if st.session_state.current_tab == 'settings' else "secondary", key="settings_btn_1"):
    ...
    if st.session_state.current_tab == 'history':
        ...
        if 'analysis_history' in st.session_state and st.session_state.analysis_history:
            for analysis in st.session_state.analysis_history:
                with st.expander(f"Analysis from {analysis.get('timestamp', 'Unknown date')}"):
                    st.write("Product Details:")
                    st.write(f"- Type: {analysis.get('product_type', 'N/A')}")
                    st.write(f"- Brand: {analysis.get('brand', 'N/A')}")
                    st.write(f"- Model: {analysis.get('model', 'N/A')}")
                    st.write(f"- Authenticity: {analysis.get('authenticity_status', 'N/A')}")
                    st.write(f"- Confidence: {analysis.get('confidence_score', 0):.2f}%")
                    
                    if analysis.get('image_data'):
                        st.image(analysis['image_data'], caption="Analyzed Product")
        else:
            st.info("No analysis history found. Start by analyzing some products!")
        ...
        if 'history_manager' in st.session_state:
            ...
            with col1:
                ...
                st.metric("Authentic Products", stats.get('authentic_count', 0))
                
            with col2:
                st.metric("Counterfeit Products", stats.get('counterfeit_count', 0))
                avg_conf = stats.get('avg_confidence', 0)
                if isinstance(avg_conf, (int, float)):
                    st.metric("Average Confidence", f"{avg_conf:.1f}%")
                else:
                    st.metric("Average Confidence", "0.0%")
                
            # Add authenticity distribution chart
            if stats.get('authenticity_distribution'):
                st.markdown("#### Authenticity Distribution")
                data = stats['authenticity_distribution']
                st.bar_chart(data)
        else:
            st.info("No analytics data available yet. Start analyzing products to see statistics!")
        ...
        confidence_threshold = st.slider(
            ...
            help="Minimum confidence level to consider a product authentic"
        )
        ...
        if st.button("Export Database"):
            if 'history_manager' in st.session_state:
                export_path = st.session_state.history_manager.export_data()
                if export_path:
                    st.success(f"✅ Database exported successfully to: {export_path}")
                else:
                    st.error("❌ Failed to export database")
                    
        if st.button("Clear History"):
            if st.session_state.get('history_manager'):
    ...
    with st.sidebar:
        ...
        st.markdown("""
            ...
            <div class='history-header'>
                <h3>📚 Product History & Database</h3>
            </div>
        """, unsafe_allow_html=True)
        
        # History navigation tabs with custom styling
        st.markdown("""
            <style>
                        ...
                        with st.spinner("🤖 AI analyzing product authenticity..."):
                            ...
                            if api_result and not api_error:
                                ...
                                try:
                                    ...
                                    # Always keep in session state for immediate display
                                    if 'analysis_history' not in st.session_state:
                                        st.session_state.analysis_history = []
        ...
        if not st.session_state.analysis_history:
            ...
            for i, product in enumerate(filtered_products):
                ...
                with st.expander(f"{status_emoji} {get_analysis_brand(product)} {product.get('model', 'Unknown')} - {product.get('timestamp', 'Unknown')}", expanded=False):
                    ...
                    with action_col2:
                        is_flagged = product.get('is_flagged', False)
                        if st.button("🚨 Flag" if not is_flagged else "🔴 Unflag", key=f"flag_main_{i}"):
                            if st.session_state.history_manager.save_flagged(product.get('id')):
                                st.success("Product flagged!" if not is_flagged else "Product unflagged!")
                                st.rerun()
                                st.warning("Product flagged!")
                                st.rerun()
